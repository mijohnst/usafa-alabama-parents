...
if (!$member && $q !== '') {
    $like = '%' . $q . '%';
    $s = $pdo->prepare("SELECT id, first_name, last_name, cadet_first_name, cadet_last_name, membership_year, membership_paid
        FROM members WHERE archived=0
        AND (first_name LIKE ? OR last_name LIKE ? OR cadet_first_name LIKE ? OR cadet_last_name LIKE ?)
        ORDER BY last_name ASC, first_name ASC LIMIT 30");
...
if ($member) {
    ...
    $cadet_full = trim(($member['cadet_first_name']??'') . ' ' . ($member['cadet_last_name']??''));
    $parent_full = trim($member['first_name'] . ' ' . $member['last_name']);
...
<table class="result-table">
  ...
  <tr>
    <td style="font-weight:600"><?= h($r['first_name'] . ' ' . $r['last_name']) ?></td>
    <td><?= h(trim(($r['cadet_first_name']??'') . ' ' . ($r['cadet_last_name']??''))) ?: '<span style="color:#c0c8d4">—</span>' ?></td>
